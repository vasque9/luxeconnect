...
export default function AdminPanel() {
  ...
  const deleteUser = async (id: string, email: string) => {
    ...
    if (!confirm('¿Estás seguro? Se borrarán todos sus datos, perfil, anuncios y pagos.')) return;
            ...
            {users?.items?.map((u: any) => (
              <div key={u.id} className="bg-card rounded-xl border border-border p-4 flex flex-wrap justify-between items-center gap-3">
                ...
                <div className="flex gap-2 items-center">
                  ...
                  {u.role !== 'ADMIN' && (
                    <button onClick={() => deleteUser(u.id, u.email)} className="px-3 py-1 rounded-lg bg-red-900/20 text-red-300 text-[10px] font-semibold border border-red-500/20">Eliminar</button>
                  )}
                </div>
              </div>
            ))}
